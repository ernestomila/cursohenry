...
//let queue = new Queue();
//let arr = [23, 43, "OUT", 65, "OUT", 32, 55, "OUT"];
function henryParking(arr) {
  // Tu código aquí:
  console.log(arr.length);
  if(arr.length === 0) return false;
  if(arr[0] == "OUT" && arr.length == 1) return false;
  let countOut = 0; 
  let countCars = 0;
  ...
  if(countOut > countCars) return false;
