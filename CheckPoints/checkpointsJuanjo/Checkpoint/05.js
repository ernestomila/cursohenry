...
function cobrarClientes(clientesQueue) {
  // Tu código aquí:
  if(clientesQueue == "") return false;
  let clientesBien = [];
  for (let i = 0; i < clientesQueue.array.length; i++) {
    if(clientesQueue.array[i].dinero >= clientesQueue.array[i].precioProductos){
      let aux = clientesQueue.dequeue();
      clientesBien.push(aux.nombre);
      i--;
    } else {
      return clientesBien;
    }
  }
  if(clientesQueue.array.length == 0) return false;
  return clientesBien;
}
// console.log("Clientes: ",cobrarClientes(clientes));
